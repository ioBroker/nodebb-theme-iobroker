...
const defaults = {
	enableQuickReply: 'on',
	hideSubCategories: 'off',
	hideCategoryLastPost: 'off',
};
...
library.init = async function (params) {
	...
	routeHelpers.setupAdminPageRoute(router, '/admin/plugins/iobroker', [], controllers.renderAdminPage);
...
library.addProfileItem = async (data) => {
	data.links.push({
		...
		route: 'theme',
		icon: 'fa-paint-brush',
		name: '[[themes/iobroker:settings.title]]',
		visibility: {
			self: true,
			other: false,
			moderator: false,
...
library.loadThemeConfig = async function (uid) {
	...
	config.hideSubCategories = config.hideSubCategories === 'on';
	config.hideCategoryLastPost = config.hideCategoryLastPost === 'on';
	return config;
};
...
library.getThemeConfig = async function (config) {
	...
	// Apply theme-specific configs for backwards compatibility
	const themeSettings = await meta.settings.get('iobroker');
	config.hideSubCategories = themeSettings.hideSubCategories === 'on';
	config.hideCategoryLastPost = themeSettings.hideCategoryLastPost === 'on';
	config.enableQuickReply = themeSettings.enableQuickReply === 'on';
	
	return config;
};

library.getAdminSettings = async function (hookData) {
	if (hookData.plugin === 'iobroker') {
		hookData.values = {
			...defaults,
			...hookData.values,
		};
